...
{
    class TcpChatChannel : IChatChannel
    {
        ...
        internal TcpChatChannel(Guid id, string displayName, IChatAuthor currentAuthor)
        {
            Id = id;
        ...
        {
            var message = new TcpChatMessage(_currentAuthor, DateTimeOffset.Now, text);
        ...
        {
            var message = new TcpChatMessage(author, DateTimeOffset.Now, text);
            Messages.Add(message);
            OnMessageReceived?.Invoke(message);
        }
    }
